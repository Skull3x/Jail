...
class Massiveeconomy extends BaseEconomy{
    ...
    public function bail(Player $player){
        ...
        $money = $this->getPlugin()->getEco()->getInstance()->getMoney($player->getName());
     		...
     		$this->getPlugin()->getEco()->getInstance()->setMoney($player->getName(), $money - $this->getPlugin()->getConfig()->get("money-per-minute")*($t[$player->getName()]["minutes"]+1));
     		$this->getPlugin()->unjail($player);
     		$player->sendMessage($this->getPlugin()->colourMessage("&aYou have been unjailed successfully!"));
     		$player->sendMessage("Bank : -" . $this->getPlugin()->getEco()->getInstance()->getMoneySymbol() . $this->getConfig()->get("money-per-minute") * ($t[$player->getName()]["minutes"]+1) . " | " . $this->getPlugin()->getEco()->getInstance()->getMoneySymbol() . $this->getPlugin()->getEco()->getInstance()->getMoney($player->getName()) . " left");
	     	return true;
    }
