...
class Economyapi extends BaseEconomy{
    ...
    public function bail(Player $player){
        ...
        $money = $this->getPlugin()->getEco()->getInstance()->myMoney($player);
	     	if($money < ($this->getConfig()->get("money-per-minute")*$t[$player->getName()]["minutes"])){
	        		$player->sendMessage($this->getPlugin()->colourMessage("&cYou don't have enough money to bail!\n&cYou need " . $this->getConfig()->get("money-per-minute")*$t[$player->getName()]["minutes"]));
	        		return false;
     		}
     		$this->getPlugin()->getEco()->getInstance()->reduceMoney($player, $this->getPlugin()->getConfig()->get("money-per-minute")*$t[$player->getName()]["minutes"]);
     		$this->getPlugin()->unjail($player);
     		$player->sendMessage($this->getPlugin()->colourMessage("&aYou have been unjailed successfully!"));
     		$player->sendMessage("Bank : -$" . $this->getConfig()->get("money-per-minute") * $t[$player->getName()]["minutes"] . " | $" . $this->getPlugin()->getEco()->getInstance()->myMoney($player) . " left");
	     	return true;
    }
